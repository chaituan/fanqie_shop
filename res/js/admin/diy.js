function  callbackupload(res,classs){
    $("." + classs ).val(res).change();
}
(function () {
    // diy类参数
    var options = {}
        ...
        , defaultData = {
            ...
            banner: {
                ...
                data: [
                    {
                        imgUrl: base_url_js + 'res/images/admin/banner_01.jpg',
                ...
                toolbar: {
                    ...
                    submit: function () {
                        $('#submit').click(function () {
                            ...
                            $.post('', {data: diyData}, function (res) {
                ...
                addItem: {
                    ...
                    banner: function (itemId, $items) {
                        var data = method.diyData.additemData(itemId, 'banner')
                            ...
                            '  <div class="container">' +
                            '    <div class="item-image"> <div class="' + data.dataId + '"><div class="fc-upload fc-frame"><div class="fc-upload-btn fq_iframe" data-url="'+site_url_js+'/adminct/widget/images/index" data-class="' + data.dataId + '" data-title="添加图片" data-shop="" data-num="1" data-wh="800px,550px"><i class="layui-icon layui-icon-camera" style="font-size: 20px;"></i></div><input name="imgName" class="thumb" type="hidden" lay-verify="thumb" value=""></div></div></div>' +
                            '    <div class="item-form am-form-file">' +
                            '        <div class="input-group">' +
                            '            <input type="text" name="imgName" data-bind="data.' + data.dataId + '.imgName"' +
                            '              class="' + data.dataId + '" value="' + data.imgName + '" placeholder="请选择图片" readonly>' +
                            '            <input type="hidden" name="imgUrl" data-bind="data.' + data.dataId + '.imgUrl"' +
                            '              class="' + data.dataId + '"  value="' + data.imgUrl + '">' +
                            '        </div>' +
                    ...
                    adimg: function (itemId, $items) {
                        var data = method.diyData.additemData(itemId, 'adimg')
                            , $html = $(
                            '<div class="item" data-key="' + data.dataId + '">' +
                            '  <div class="container">' +
                            '    <div class="item-image"> <div class="' + data.dataId + '"><div class="fc-upload fc-frame"><div class="fc-upload-btn fq_iframe" data-url="'+site_url_js+'/adminct/widget/images/index" data-class="' + data.dataId + '" data-title="添加图片" data-shop="" data-num="1" data-wh="800px,550px"><i class="layui-icon layui-icon-camera" style="font-size: 20px;"></i></div><input name="imgName" class="thumb" type="hidden" lay-verify="thumb" value=""></div></div></div>' +
                     ...
                     * @param $items
                     */
                    nav: function (itemId, $items) {
                        var data = method.diyData.additemData(itemId, 'nav')
                            , $html = $(
                                '<div class="item" data-key="' + data.dataId + '">' +
                                '  <div class="container">' +
                                '    <div class="item-image"> <div class="' + data.dataId + '"><div class="fc-upload fc-frame"><div class="fc-upload-btn fq_iframe" data-url="' + site_url_js + '/adminct/widget/images/index" data-class="' + data.dataId + '" data-title="添加图片" data-shop="" data-num="1" data-wh="800px,550px"><i class="layui-icon layui-icon-camera" style="font-size: 20px;"></i></div><input name="imgName" class="thumb" type="hidden" lay-verify="thumb" value=""></div></div></div>' +
                                '    <div class="item-form am-form-file">' +
                                '        <div class="input-group">'+
                                '           <input type="text" name="name" data-bind="data.' + data.dataId + '.name"'+
                                '               value="' + data.name +'" placeholder="请输入菜单中文名称">'+
                                '        </div>'+
                                '        <div class="input-group margin-top-sm">' +
                                '            <input type="text" name="imgName" data-bind="data.' + data.dataId + '.imgName"' +
                                '              class="' + data.dataId + '" value="' + data.imgName + '" placeholder="请选择图片" readonly>' +
