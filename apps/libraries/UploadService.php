...
class UploadService {
     ...
     * @return mixed
     */
    protected static function successful($path, $fileInfo) {
    ...
    public static function image($fileName, $path, $moveName = true, $autoValidate=true, $root=null, $rule='uniqid') {
        ...
        $imageValidate = ['max_size'=>2097152,'allowed_types'=>'jpg|jpeg|png|mp4','upload_path'=>$dir,'file_name'=>uniqid(time())];
        $CI->upload->initialize($imageValidate);
        $r = $CI->upload->do_upload($fileName);
        if($autoValidate){
        	if(!$r)AjaxResult_error($CI->upload->display_errors());
        } 
        $info = $CI->upload->data();
        //移动文件
        if (!(move_uploaded_file($file["tmp_name"],$info['full_path']))) { //移动失败
        	AjaxResult_error('移动文件失败');
        } 
        $path = str_replace(DS, '/', DS.$path.DS.$info['file_name']);
        $fileInfo = ['name'=>$info['file_name'],'att_dir'=>$path,'att_size'=>$info['file_size'],'att_type'=>$info['image_type'],'time'=>time()];
        return $fileInfo;
    }
