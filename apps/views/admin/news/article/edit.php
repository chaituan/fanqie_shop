<?php echo template('admin/headers');?>
    <form class="layui-form layui-card-body" method="post">
            ...
            <div class="layui-input-block">
                <select name="data[group_id]"  >
                    <?php foreach($parent as $v){?>
                        <option value="<?php echo $v['id'];?>" <?php if($v['id']==$item['group_id']) echo "selected";?>>  <?php echo str_repeat('├',$v['level']).' '.$v['gname'];?></option>
                    <?php }?>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="data[s_title]" class="layui-input" value="<?php echo $item['s_title'] ?>" placeholder="如果不填写则默认读取文章标题" >
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">分享简介</label>
            <div class="layui-input-block">
                <input type="text" name="data[s_info]" class="layui-input" value="<?php echo $item['s_info'] ?>" placeholder="如果不填写则默认读取文章标题">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">分享图</label>
            <div class="layui-input-block">
                <?php echo admin_btn("data[s_thumb]",'upload',"s_thumb",$item['s_thumb'],1,1);?><div class="thumb-say">不上传默认为封面图</div>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">封面图</label>
            <div class="layui-input-block">
                <?php echo admin_btn("data[thumb]",'upload',"thumbs",$item['thumb'],1,1);?>
            </div>
        </div>
        <div class="layui-form-item">
